...
void Window::onCreate() {
  ...
  int numObstacles = 3; // Number of obstacles, can be adjusted
...
void Window::checkCollisionWithObstacles() {
  ...
  for (auto const &obstacle : m_obstacles) {
    ...
    if (distance < (scaledBallRadius + obstacle.radius)) {
...
void Window::setupFlippers() {
  // Initialize left and right flippers with position, angle, and length
  m_leftFlipper = {{-0.5f, -0.8f}, -0.5f, 2.5f};
  m_rightFlipper = {{0.5f, -0.8f}, -0.5f, 2.5f};
}

void Window::renderFlipper(Flipper const &flipper, bool isLeft) {
  glBindVertexArray(m_VAO);

  // Define vertex positions for the flipper with increased thickness
  std::array<glm::vec2, 4> positions;
  if (isLeft) {
    positions = {glm::vec2{0.0f, -0.1f}, glm::vec2{flipper.length, -0.1f},
                 glm::vec2{flipper.length, 0.1f}, glm::vec2{0.0f, 0.1f}};
  } else {
    positions = {glm::vec2{0.0f, -0.1f}, glm::vec2{-flipper.length, -0.1f},
                 glm::vec2{-flipper.length, 0.1f}, glm::vec2{0.0f, 0.1f}};
  }

  // Set flipper color to white
  ...
  // Set rotation angle (inverting for the right flipper)
  float angle = isLeft ? flipper.angle : -flipper.angle;
  ...
  // Draw the flipper as a filled shape
  glDrawArrays(GL_TRIANGLE_FAN, 0, positions.size());
...
void Window::onUpdate() {
  ...
  m_ball.velocity.y -= 1.0f * deltaTime;
...
void Window::onEvent(SDL_Event const &event) {
  if (event.type == SDL_KEYDOWN) {
    ...
    if (event.key.keysym.sym == SDLK_LEFT)
      m_leftFlipper.angle = 0.8f;
    if (event.key.keysym.sym == SDLK_RIGHT)
      m_rightFlipper.angle = 0.8f;
  } else if (event.type == SDL_KEYUP) {
    // Reset flipper angles when keys are released
    if (event.key.keysym.sym == SDLK_LEFT)
      m_leftFlipper.angle = -0.5f;
    if (event.key.keysym.sym == SDLK_RIGHT)
      m_rightFlipper.angle = -0.5f;
  }
}
